...
import kafka.metrics.ClientMetricsCache.{DEFAULT_TTL_MS, cmCache}
...
class ClientMetricsCache(maxSize: Int) {
  ...
  def get(id: Uuid): Option[ClientMetricsInstanceState] =  {
  ...
  def add(instance: ClientMetricsInstanceState)= {
    _cache.synchronized{
  ...
  private def update(oldSubscription: SubscriptionInfo, newSubscription: SubscriptionInfo) = {
    _cache.entrySet().forEach(element =>  {
      ...
      if (oldSubscription!= null && clientInstance.getClientInfo.isMatched(Some(oldSubscription.getClientMatchingPatterns))) {
        updatedMetricSubscriptions.remove(oldSubscription)
      }
      if (newSubscription != null && clientInstance.getClientInfo.isMatched(Some(newSubscription.getClientMatchingPatterns))){
        updatedMetricSubscriptions.add(newSubscription)
      }
      element.getValue.replace(ClientMetricsInstanceState(clientInstance, updatedMetricSubscriptions))
    })
  }

  private def isExpired(element: ClientMetricsInstanceState) = {
   ...
   * @param instance
   */
  class ClientMetricsCacheValue(instance: ClientMetricsInstanceState) {
    var clientInstance :ClientMetricsInstanceState = instance
    def getClientInstance = clientInstance
    def replace(instance: ClientMetricsInstanceState): Unit = {
