...
import ClientMetricsConfig.ClientMetrics.DEFAULT_INTERVAL_MS
import ClientMetricsConfig.SubscriptionInfo
 ...
 */
object ClientMetricsInstanceState {

  def apply(instance: ClientMetricsInstanceState,
            subscriptions: java.util.Collection[SubscriptionInfo]): ClientMetricsInstanceState = {
  ...
  private def create(id: Uuid,
    ...
    new ClientMetricsInstanceState(id,  clientInfo, targetSubscriptions,
                              targetMetrics.toList, pushInterval, allMetricsSubscribed)
  }

}

class ClientMetricsInstanceState private(clientInstanceId: Uuid,
                                         clientInfo: ClientMetricsMetadata,
  ...
  def getLastAccessTs: Long = lastAccessTs
  def getSubscriptionId: Int =  subscriptionId
  def getId: Uuid = clientInstanceId
  def getClientInfo: ClientMetricsMetadata = clientInfo
  def getSubscriptions: util.Collection[SubscriptionInfo] = subscriptions
  def getMetrics: Seq[String] = metrics
  def getAllMetricsSubscribed: Boolean = allMetricsSubscribed
  def isClientTerminating: Boolean = this.terminating
