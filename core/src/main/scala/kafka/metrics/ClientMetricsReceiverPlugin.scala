...
object ClientMetricsReceiverPlugin {
  ...
  def isEmpty: Boolean = cmReceivers.isEmpty

  def add(receiver: ClientTelemetryReceiver): cmReceivers.type = {
    cmReceivers.append(receiver)
  }

  def getPayLoad(request: PushTelemetryRequest) : ClientMetricsPayload = {
     new ClientMetricsPayload(request.data().clientInstanceId(), request.data().terminating(),
