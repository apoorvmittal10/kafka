...
import kafka.metrics.ClientMetricsTestUtils.{createCMSubscription, getClientInstance, getSerializedMetricsData, setupClientMetricsPlugin}
import ClientMetricsConfig1.ClientMatchingParams.{CLIENT_SOFTWARE_NAME, CLIENT_SOFTWARE_VERSION}
import ClientMetricsConfig1.ClientMetrics
import org.apache.kafka.common.Uuid
import org.apache.kafka.common.message.{PushTelemetryRequestData}
import org.apache.kafka.common.protocol.Errors
import org.apache.kafka.common.record.CompressionType
import org.apache.kafka.common.requests.{GetTelemetrySubscriptionsResponse, PushTelemetryResponse}
...
class ClientMetricsRequestResponseTest {
  ...
  private def sendGetSubscriptionRequest(clientInfo: ClientMetricsMetadata,
...
//    getCM.processGetSubscriptionRequest(request, clientInfo, 20)
  ...
  private def sendPushTelemetryRequest(data: PushTelemetryRequestData, clientInfo: ClientMetricsMetadata, delay: Int = 0): PushTelemetryResponse = {
...
//    getCM.processPushTelemetryRequest(request, null, clientInfo, 5000)
  ...
  @Test def testGetClientMetricsRequestAndResponse(): Unit = {
    ...
    val clientInfo = ClientMetricsMetadata("testClient1", "clientId3", "Java", "11.1.0", "192.168.1.7", "9093")
...
//      assertTrue(ClientMetricsManager.getSupportedCompressionTypes.contains(x))
  ...
  @Test def testRequestAndResponseWithNoMatchingMetrics(): Unit = {
    ...
    val clientInfo = ClientMetricsMetadata("testClient1", "clientId3", "Python", "11.1.0", "192.168.1.7", "9093")
    ...
    assertEquals(response.pushIntervalMs(), ClientMetrics.DEFAULT_INTERVAL_MS)
    ...
    props.put(ClientMetricsConfig1.ClientMetrics.ClientMatchPattern, clientMatch.mkString(","))
...
//    props.put(ClientMetricsConfig.ClientMetrics.AllMetricsFlag, "true")
  ...
  @Test def testRequestWithDisabledClient(): Unit = {
    val clientInfo = ClientMetricsMetadata("testClient5", "clientId5", "Java", "11.1.0", "192.168.1.7", "9093")
  ...
  def testPushMetricsRequestParameters(): Unit = {
    val props = new Properties()
    val pushInterval = 10
    props.put(ClientMetricsConfig1.ClientMetrics.PushIntervalMs, pushInterval.toString)
    val subscription = createCMSubscription("cm_1", props)
    assertNotNull(subscription)

    val clientInfo = ClientMetricsMetadata("testClient1", "clientId1", "Java", "11.1.0", "192.168.1.7", "9093")
    val response = sendGetSubscriptionRequest(clientInfo).data()
    assertNotNull(response)
    val cmClient = getClientInstance(response.clientInstanceId())
    assertNotNull(cmClient)
