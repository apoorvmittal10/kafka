...
import org.apache.kafka.clients.{ClientTelemetrySubscription, CommonClientConfigs}
...
class ClientTelemetryTest extends IntegrationTestHarness {
  ...
  def testBasic(): Unit = {
    ...
    try {
      val clientTelemetryReporter = producer.asInstanceOf[KafkaProducer[ByteArraySerializer, ByteArraySerializer]].clientTelemetry().get()
      assertNotNull(clientTelemetryReporter)

      val latch = new CountDownLatch(1)

//      val clientTelemetrySender = clientTelemetryReporter.clientSender()
  ...
  def enableClientMetricsConfigs(pushInterval: Option[Int] = Some(1000)): Unit = {
    ...
    try {
      ...
      if (pushInterval.isDefined)
        list.add(new AlterConfigOp(new ConfigEntry(ClientMetricsConfig1.ClientMetrics.PushIntervalMs, pushInterval.get.toString), AlterConfigOp.OpType.SET))
