...
import org.apache.kafka.common.telemetry.internals.ClientTelemetryReporter.ClientTelemetrySubscription
...
class ClientTelemetryTest extends IntegrationTestHarness {
  ...
  @Test
  def testBasic(): Unit = {
    createClientMetricsConfigs()

    val subscriptions = ListBuffer[ClientTelemetrySubscription]()
    val emitted = new ListBuffer[util.List[SinglePointMetric]]()
    val producer = createProducer()

    try {
//      val clientTelemetryReporter = producer.asInstanceOf[KafkaProducer[ByteArraySerializer, ByteArraySerializer]].clientTelemetryReporter()
//      assertNotNull(clientTelemetryReporter)

      val latch = new CountDownLatch(1)

//      val clientTelemetrySender = clientTelemetryReporter.telemetrySender()
//      clientTelemetrySender.addListener(new ClientTelemetryListener {
...
//        }
//
//        override def pushRequestCreated(clientInstanceId: Uuid, subscriptionId: Int, terminating: Boolean, compressionType: CompressionType, metrics: util.List[SinglePointMetric]): Unit = {
...
//  }
  ...
  def disablePushProperties(): Properties = {
    ...
    properties.put(AdminClientConfig.ENABLE_METRICS_PUSH_CONFIG, "false")
    properties
  }

  def createClientMetricsConfigs(pushInterval: Option[Int] = Some(1000)): Unit = {
    val adminClient = createAdminClient()

    try {
      val configs = new util.HashMap[ConfigResource, util.Collection[AlterConfigOp]]

      val configResource = new ConfigResource(ConfigResource.Type.CLIENT_METRICS, "test-subscription")
      val list = new util.ArrayList[AlterConfigOp]()

      if (pushInterval.isDefined)
        list.add(new AlterConfigOp(new ConfigEntry(ClientMetricsConfigs.PUSH_INTERVAL_MS, pushInterval.get.toString), AlterConfigOp.OpType.SET))
        list.add(new AlterConfigOp(new ConfigEntry(ClientMetricsConfigs.SUBSCRIPTION_METRICS, ClientMetricsConfigs.ALL_SUBSCRIBED_METRICS_CONFIG), AlterConfigOp.OpType.SET))
      configs.put(configResource, list)
