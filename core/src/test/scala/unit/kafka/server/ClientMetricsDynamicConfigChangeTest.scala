...
import kafka.metrics.{ClientMetricsConfig1, ClientMetricsTestUtils}
import kafka.metrics.ClientMetricsConfig1.ClientMatchingParams.{CLIENT_SOFTWARE_NAME, CLIENT_SOFTWARE_VERSION}
...
class ClientMetricsDynamicConfigChangeTest extends KafkaServerTestHarness {
  ...
  def testClientMetricsConfigChange(quorum: String): Unit = {
    ...
    updateClientSubscription(configEntityName, props,
      () =>  ClientMetricsConfig1.getClientSubscriptionInfo(configEntityName) != null)
    val cmSubscription = ClientMetricsConfig1.getClientSubscriptionInfo(configEntityName)
  ...
  def testClientMetricsConfigMultipleUpdates(quorum: String): Unit = {
    ...
    props.put(ClientMetricsConfig1.ClientMetrics.SubscriptionMetrics, updatedMetrics)
    updateClientSubscription(configEntityName, props,
      () => ClientMetricsConfig1.getClientSubscriptionInfo(configEntityName).getSubscribedMetrics.size == 1)
    assertTrue(ClientMetricsConfig1.getClientSubscriptionInfo(configEntityName)
...
//    props.put(ClientMetricsConfig.ClientMetrics.DeleteSubscription, "true")
    updateClientSubscription(configEntityName, props,
      () => ClientMetricsConfig1.getClientSubscriptionInfo(configEntityName) == null)
    assertTrue(ClientMetricsConfig1.getClientSubscriptionInfo(configEntityName) == null)
  }
  ...
  def testClientMetricsRestartServer(quorum: String): Unit = {
    ...
    assertTrue(ClientMetricsConfig1.getSubscriptionsCount == 0)
...
//    props.put(ClientMetricsConfig.ClientMetrics.AllMetricsFlag, "true")
    ...
    assertTrue(cmSubscription.getPushIntervalMs == ClientMetricsConfig1.ClientMetrics.DEFAULT_INTERVAL_MS)
//    assertTrue(cmSubscription.getAllMetricsSubscribed)
    assertTrue(cmSubscription.getSubscribedMetrics.size == 1)
    assertTrue(cmSubscription.getSubscribedMetrics.mkString(",").equals(""))
  }
