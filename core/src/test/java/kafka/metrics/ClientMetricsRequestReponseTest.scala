...
//  private def sendGetSubscriptionRequest(clientInfo: ClientMetricsInstanceMetadata,
//                                         id: Uuid = Uuid.ZERO_UUID): GetTelemetrySubscriptionsResponse = {
////    val request = new GetTelemetrySubscriptionsRequest.Builder(
////      new GetTelemetrySubscriptionsRequestData().setClientInstanceId(id)).build(0);
////    getCM.processGetSubscriptionRequest(request, clientInfo, 20)
//    null
//  }

//  private def sendPushTelemetryRequest(data: PushTelemetryRequestData, clientInfo: ClientMetricsInstanceMetadata, delay: Int = 0): PushTelemetryResponse = {
...
//    val subscription1 = ClientMetricsTestUtils.createClientMetricsSubscription("cm_1", null)
//    assertNotNull(subscription1)
//
//    val clientInfo = new ClientMetricsInstanceMetadata("testClient1", "clientId3", "Java", "11.1.0", "192.168.1.7", "9093")
...
//    val cmClient = ClientMetricsTestUtils.getClientMetricsInstance(clientInstanceId)
...
//    assertEquals(response.pushIntervalMs(), cmClient.pushIntervalMs)
//    assertEquals(response.subscriptionId(), cmClient.subscriptionId)
//
//    assertEquals(response.requestedMetrics().size, cmClient.metrics.size)
//    response.requestedMetrics().forEach(x => assertTrue(cmClient.metrics.contains(x)))
...
//    val subscription1 = ClientMetricsTestUtils.createClientMetricsSubscription("cm_2")
//    assertNotNull(subscription1)

    // Create a python client that do not have any matching subscriptions.
//    val clientInfo = new ClientMetricsInstanceMetadata("testClient1", "clientId3", "Python", "11.1.0", "192.168.1.7", "9093")
//    var response = sendGetSubscriptionRequest(clientInfo).data()
//    var cmClient = ClientMetricsTestUtils.getClientMetricsInstance(response.clientInstanceId())
//    val clientInstanceId = response.clientInstanceId()
//
//    // Push interval must be set to the default push interval and requested metrics list should be empty
//    assertEquals(response.pushIntervalMs(), ClientMetricsConfigs.DEFAULT_INTERVAL_MS)
//    assertNotEquals(response.pushIntervalMs(), subscription1.intervalMs())
//    assertEquals(response.subscriptionId(), cmClient.subscriptionId)
...
//    val clientMatch = List(s"${ClientMetricsConfigs.CLIENT_SOFTWARE_NAME}=Python", s"${ClientMetricsConfigs.CLIENT_SOFTWARE_VERSION}=11.1.*")
//    props.put(ClientMetricsConfigs.CLIENT_MATCH_PATTERN, clientMatch.mkString(","))
...
//    cmClient = ClientMetricsTestUtils.getClientMetricsInstance(clientInstanceId)
//    assertEquals(response.pushIntervalMs(), subscription2.intervalMs())
//    assertEquals(response.subscriptionId(), cmClient.subscriptionId)
//    assertTrue(!response.requestedMetrics().isEmpty)
//    response.requestedMetrics().forEach(x => assertTrue(cmClient.metrics.contains(x)))
  }
...
//    ClientMetricsTestUtils.createClientMetricsSubscription("subscription1")
//
...
//    val subscription1 = ClientMetricsTestUtils.createClientMetricsSubscription("cm_all_metrics", props)
//    assertNotNull(subscription1)
//
//    val response = sendGetSubscriptionRequest(clientInfo).data()
//
...
//    val cmClient = ClientMetricsTestUtils.getClientMetricsInstance(response.clientInstanceId())
//    assertNotNull(cmClient)
//    assertEquals(response.pushIntervalMs(), cmClient.pushIntervalMs)
//    assertEquals(response.subscriptionId(), cmClient.subscriptionId)
...
//    val clientInfo = new ClientMetricsInstanceMetadata("testClient5", "clientId5", "Java", "11.1.0", "192.168.1.7", "9093")
//
//    val subscription1 = ClientMetricsTestUtils.createClientMetricsSubscription("cm_4")
//    assertNotNull(subscription1)
//
//    // Submit a request to get the subscribed metrics
//    var response = sendGetSubscriptionRequest(clientInfo).data()
//    val clientInstanceId = response.clientInstanceId()
//    var cmClient = ClientMetricsTestUtils.getClientMetricsInstance(clientInstanceId)
//    assertNotNull(cmClient)
//
//    val oldSubscriptionId = response.subscriptionId()
//    assertEquals(response.pushIntervalMs(), subscription1.intervalMs())
//    assertEquals(response.subscriptionId(), cmClient.subscriptionId)
//    response.requestedMetrics().forEach(x => assertTrue(subscription1.metrics().contains(x)))
//
//    // Now create a new client subscription with push interval set to 0.
//    val props = new Properties()
//    props.put(ClientMetricsConfigs.PUSH_INTERVAL_MS, 0.toString)
//    val subscription2 = ClientMetricsTestUtils.createClientMetricsSubscription("cm_update_2_disable", props)
//    assertNotNull(subscription2)
//
...
//    val subscription = ClientMetricsTestUtils.createClientMetricsSubscription("cm_1", props)
//    assertNotNull(subscription)
//
//    val clientInfo = new ClientMetricsInstanceMetadata("testClient1", "clientId1", "Java", "11.1.0", "192.168.1.7", "9093")
//    val response = sendGetSubscriptionRequest(clientInfo).data()
//    assertNotNull(response)
//    val cmClient = ClientMetricsTestUtils.getClientMetricsInstance(response.clientInstanceId())
//    assertNotNull(cmClient)
...
//    assertNotNull(pushResponse)
//    assertEquals(pushResponse.error().code(), Errors.UNKNOWN_SUBSCRIPTION_ID.code())
//
...
//    ClientMetricsTestUtils.createClientMetricsSubscription("cm_1", props)
...
////    data.setClientInstanceId(response2.clientInstanceId())
////    data.setSubscriptionId(response2.subscriptionId())
////    ClientMetricsTestUtils.getClientMetricsInstance(response2.clientInstanceId()).terminating(true)
//
...
//    val plugin = ClientMetricsTestUtils.setUpClientMetricsPlugin()
//    val props = new Properties()
//    val pushInterval = 10
//    props.put(ClientMetricsConfigs.PUSH_INTERVAL_MS, pushInterval.toString)
//    val subscription = ClientMetricsTestUtils.createClientMetricsSubscription("cm_2", props)
//    assertNotNull(subscription)
//
//    val clientInfo = new ClientMetricsInstanceMetadata("testClient1", "clientId1", "Java", "11.1.0", "192.168.1.7", "9093")
//    val response = sendGetSubscriptionRequest(clientInfo).data()
...
//    var pushResponse = sendPushTelemetryRequest(data, clientInfo, pushInterval)
//    assertEquals(pushResponse.error().code(), Errors.NONE.code())
//    assertEquals(plugin.exportMetricsInvokedCount, 0)
...
//    pushResponse = sendPushTelemetryRequest(data, clientInfo, pushInterval)
//    assertEquals(pushResponse.error().code(), Errors.NONE.code())
//    assertEquals(plugin.exportMetricsInvokedCount, 1)
...
//    assertNotNull(plugin.metricsDataAsCopy().array())
  }
...
//    val pushInterval = 5
//    props.put(ClientMetricsConfigs.PUSH_INTERVAL_MS, pushInterval.toString)
//    val subscription = ClientMetricsTestUtils.createClientMetricsSubscription("cm_5", props)
//    assertNotNull(subscription)
//
//    val clientInfo = new ClientMetricsInstanceMetadata("testClient1", "clientId1", "Java", "11.1.0", "192.168.1.7", "9093")
//    val response = sendGetSubscriptionRequest(clientInfo).data()
...
//      val (compressedData, metricStr) = ClientMetricsTestUtils.getSerializedMetricsData(x, metricsMap)
