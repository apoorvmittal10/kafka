...
//    }
    ...
    protected SinglePointMetric newTelemetryMetric(MetricName metricName, MetricType metricType, double value) {
        if (metricType == MetricType.SUM) {
            return SinglePointMetric.sum(new MetricKey(metricName), value, true, Instant.ofEpochMilli(MOCK_TIME.milliseconds()));
        }
