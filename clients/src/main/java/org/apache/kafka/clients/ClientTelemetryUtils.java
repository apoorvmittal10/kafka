...
import static org.apache.kafka.clients.ClientTelemetryReporter.MAX_TERMINAL_PUSH_WAIT_MS;
...
import org.apache.kafka.clients.ClientTelemetryReporter.ConnectionErrorReason;
...
public class ClientTelemetryUtils {
    ...
    static {
        ...
        CONSUMER_CONFIG_MAPPING.put(ConsumerConfig.CLIENT_RACK_CONFIG, ClientTelemetryProvider.CLIENT_RACK);
...
//        clientTelemetryReporter.initiateClose(d);
    ...
    public static Predicate<? super MetricKeyable> getSelectorFromRequestedMetrics(List<String> requestedMetrics) {
        log.info("[APM] getSelectorFromRequestedMetrics {}", requestedMetrics);
    ...
    public static ByteBuffer serialize(byte[] raw, CompressionType compressionType) {
        ...
        if (compressionType == CompressionType.ZSTD) {
            buffer.position(AbstractRecords.recordBatchHeaderSizeInBytes(RecordBatch.CURRENT_MAGIC_VALUE, compressionType));
        }
        try (ByteBufferOutputStream compressedOut = new ByteBufferOutputStream(buffer)) {
//            compressedOut.position(AbstractRecords.recordBatchHeaderSizeInBytes(RecordBatch.CURRENT_MAGIC_VALUE, compressionType));
            try (OutputStream out = compressionType.wrapForOutput(compressedOut, RecordBatch.CURRENT_MAGIC_VALUE)) {

//                DataOutputStream stream = new DataOutputStream(out);
//                Utils.writeTo(stream, buffer, raw.length * 2);
                out.write(raw);
                out.flush();

                ByteBuffer buffer1 = compressedOut.buffer();

                if (compressionType == CompressionType.NONE || compressionType == CompressionType.SNAPPY) {
                    buffer1.flip();
                }
